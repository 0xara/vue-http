...
class unfetchDriver {
    ...
    handle() {
        return import(/* webpackChunkName: "vendors/unfetch" */'unfetch').then(({ default: fetch }) => {
    ...
    static prepareSettings(options) {
        ...
        const restOptions = unfetchDriver.handleXhr(rest);

        const settings = {
            method, url: unfetchDriver.prepareUrl(method, url, data), body: unfetchDriver.prepareData(method, data), ...restOptions
        };

        settings.headers = { ...(settings.headers || {}), ...unfetchDriver.prepareDataType(dataType) };
